...
class Main_Window():
    def __init__(self):
        ...
        self.ref_menu.add_command(label="Жанры",command=self.open_win)
        self.ref_menu.add_command(label="Название книги",command=self.open_win)
        self.ref_menu.add_command(label="Инвентарный номер",command=self.open_win)
        self.ref_menu.add_command(label="Авторы",command=self.open_win)
        self.ref_menu.add_command(label="Год издания",command=self.open_win)
        self.ref_menu.add_command(label="Издательства",command=self.open_win)
        self.ref_menu.add_command(label="Места издания",command=self.open_win)
        self.ref_menu.add_command(label="Количество страниц",command=self.open_win)
        self.ref_menu.add_command(label="Цена",command=self.open_win)
        self.ref_menu.add_command(label="Инвентарный номер",command=self.open_win)

        self.book_menu.add_command(label='Поступление книг')
        self.book_menu.add_command(label='Список книг') 
        self.book_menu.add_command(label='Выдача книг')
        self.ref_menu.add_command(label="Возврат книг")

        self.otch_menu.add_command(label="Отчет по выдаче книг",)
        self.otch_menu.add_command(label="Отчет по возврату книг")
    ...
    def open_name_book(self):
        '''метод Открыть окно Издательства'''
        self.root.withdraw() #скрыть окно
        open_name_book()
    
    def open_inventory_number(self):
        '''метод Открыть окно Места издания'''
        self.root.withdraw() #скрыть окно
        inventory_number()
    
    def open_win_name_author(self):
        '''метод Открыть окно Авторы'''
        self.root.withdraw() #скрыть окно
        Name_author_Window()
    
    def open_win_genre(self):
        '''метод Открыть окно Жанры'''
        self.root.withdraw() #скрыть окно
        Genre_Window()
...
class open_win():
    ...
    def __init__(self):
        ...
        self.root2.title("Студенческая библиотека/Жанры")
...
class open_name_book(open_win):
    '''Окно Названик книг'''
    def __init__(self):
        self.root2 = tk.Tk()
        self.root2.geometry("800x500")
        self.root2.title("Студентечская библиотека/Название Книг")
        self.root2.protocol('WM_DELETE_WINDOW', lambda: self.quit_win()) #перехват кнопки Х
        self.main_view = win
        self.db = db
        ...
        #таблица
        self.table_pr = ttk.Treeview(self.table_frame, columns=('name_book'), 
                                 height=15, show='headings')
        self.table_pr.column("name_book", width=150, anchor=tk.NW)

        self.table_pr.heading("name_book", text='Наименование книги')
...
class Name_author_Window(open_win):
    '''Окно Авторы'''
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.root2.title("Книжный магазин/Авторы")

        #таблица
        self.table_pr = ttk.Treeview(self.table_frame, columns=('name_author'), 
                                 height=15, show='headings')
        self.table_pr.column("name_author", width=150, anchor=tk.NW)
        self.table_pr.heading("name_author", text='ФИО автора')
        self.table_pr.place(relx=0, rely=0, relheight=0.9, relwidth=0.97)

        #поля для ввода
        self.lname = tk.Label(self.add_edit_frame, text="ФИО автора")
        self.lname.place(relx=0.04, rely=0.02, relheight=0.05, relwidth=0.4)
        self.ename = ttk.Entry(self.add_edit_frame)
        self.ename.place(relx=0.45, rely=0.02, relheight=0.05, relwidth=0.5)

class Genre_Window(open_win):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.root2.title("Книжный магазин/Жанры")

        #таблица
        self.table_pr = ttk.Treeview(self.table_frame, columns=('name_genre'), 
                                 height=15, show='headings')
        self.table_pr.column("name_genre", width=150, anchor=tk.NW)
        self.table_pr.heading("name_genre", text='Жанр')
        self.table_pr.place(relx=0, rely=0, relheight=0.9, relwidth=0.97)

        #поля для ввода
        self.lname = tk.Label(self.add_edit_frame, text="Жанр")
        self.lname.place(relx=0.04, rely=0.02, relheight=0.05, relwidth=0.4)
        self.ename = ttk.Entry(self.add_edit_frame)
        self.ename.place(relx=0.45, rely=0.02, relheight=0.05, relwidth=0.5)
